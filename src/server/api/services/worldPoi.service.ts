...
export async function interact(
  ...
  const poi = await db.outerCityPOI.findUnique({
    ...
    case "resource": {
      ...
      if (poi.resourceType && poi.resourceAmount > 0) {
        await db.player.update({
          where: { id: player.id },
          data: {
            ...(poi.resourceType === "gold" ? { gold: { increment: poi.resourceAmount } } : {}),
            ...(poi.resourceType === "wood" ? { wood: { increment: poi.resourceAmount } } : {}),
            ...(poi.resourceType === "stone" ? { stone: { increment: poi.resourceAmount } } : {}),
            ...(poi.resourceType === "food" ? { food: { increment: poi.resourceAmount } } : {}),
          },
        });
...
export async function harvest(
  ...
  // ===== 商队：随机交易 =====
  if (poi.type === "caravan") {
    return handleCaravan(db, hero, poi, player);
  }

  // ===== 资源点：采集资源 =====
  return handleResourceHarvest(db, hero, poi, player);
}

// ===== refreshResources: 刷新资源 =====
export async function refreshResources(db: FullDbClient) {
  // 刷新所有资源点
  const resourcePOIs = await db.outerCityPOI.findMany({
    where: { type: "resource" },
  });
  ...
  // 刷新已征服的驻军/巢穴
  const now = new Date();
  const defeatedPOIs = await db.outerCityPOI.findMany({
    where: {
      isDefeated: true,
      respawnsAt: { lte: now },
    },
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleCaravan(db: FullDbClient, hero: any, poi: any, player: any) {
  const staminaCost = 5;
  if (hero.stamina < staminaCost) {
    throw new TRPCError({ code: "BAD_REQUEST", message: "体力不足" });
  }
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleResourceHarvest(db: FullDbClient, hero: any, poi: any, player: any) {
  // 检查资源是否可采集
  ...
  // 更新玩家资源
  const resourceUpdates: Record<string, { increment: number }> = {};
  if (poi.resourceType === "gold") resourceUpdates.gold = { increment: harvestAmount };
  if (poi.resourceType === "wood") resourceUpdates.wood = { increment: harvestAmount };
  if (poi.resourceType === "stone") resourceUpdates.stone = { increment: harvestAmount };
  if (poi.resourceType === "food") resourceUpdates.food = { increment: harvestAmount };
