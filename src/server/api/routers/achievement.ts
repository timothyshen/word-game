...
export const achievementRouter = createTRPCRouter({
  ...
  getAll: protectedProcedure.query(async ({ ctx }) => {
    ...
    // 获取统计数据
    const exploredAreasCount = await ctx.db.exploredArea.count({
    ...
    // 计算当前进度
    const getProgress = (condition: Achievement["condition"]): number => {
    ...
    return ACHIEVEMENTS.filter((a) => !a.hidden).map((achievement) => {
      const progress = getProgress(achievement.condition);
  ...
  claim: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const existingClaim = player.achievements.find(
        (a) => a.achievementId === input.achievementId
      );
      if (existingClaim) {
      ...
      // 记录领取
      await ctx.db.playerAchievement.create({
        data: {
          playerId: player.id,
      ...
      await ctx.db.player.update({
        ...
        data: {
          ...
          crystals: player.crystals + (rewards.crystals ?? 0),
          exp: player.exp + (rewards.exp ?? 0),
        },
      });
      ...
      if (rewards.cardRarity) {
        ...
        if (cards.length > 0) {
          ...
          cardReward = { name: card.name, rarity: card.rarity };
        }
      }
  ...
  getByCategory: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      return ACHIEVEMENTS.filter((a) => a.category === input.category && !a.hidden).map(
        (achievement) => {
          const progress = getProgress(achievement.condition);
          const isCompleted = progress >= achievement.condition.value;
          const isClaimed = claimedIds.has(achievement.id);
