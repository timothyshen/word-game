...
export const eventsRouter = createTRPCRouter({
  // 处理事件选择
  handleChoice: publicProcedure
    .input(
      z.object({
        heroId: z.string(),
        eventType: z.string(),
        action: z.string(),
        eventData: z.string().optional(), // JSON encoded event data
      })
    )
    .mutation(async ({ ctx, input }) => {
      ...
      let result: { success: boolean; message: string; rewards?: Record<string, number> } = {
        success: true,
        message: "",
      };
        ...
        case "fight":
          if (eventData?.monster) {
            ...
            if (won) {
              ...
              // 英雄受伤
              const damage = Math.floor(eventData.monster.attack * 0.5);
