...
import { addCardEntity } from "./card-entity-utils";
 ...
 * Uses Entity system to store the player card.
...
export async function grantRandomCard(
  ...
  await addCardEntity(db, entities, playerId, card.id, 1);
...
export async function grantRandomCards(
  ...
  for (let i = 0; i < count; i++) {
    const result = await grantRandomCard(db, entities, playerId, rarity);
...
export async function openChest(
  ...
  for (let i = 0; i < draws; i++) {
    ...
    const card = await grantRandomCard(db, entities, playerId, rarity);
