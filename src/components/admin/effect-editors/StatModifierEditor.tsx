...
export function StatModifierEditor({ value, onChange }: Props) {
  ...
  return (
    <div className="space-y-2">
      ...
      <button type="button" onClick={() => onChange([...value, { stat: "attack" as StatKey, value: 0, type: "flat" }])} className={addBtnCls}>+ 添加修改器</button>
    </div>
  );
}
...
export function StatModifierField({ name, defaultValue }: { name: string; defaultValue: string }) {
  const [items, setItems] = useState<StatModifier[]>(() => {
    try { return JSON.parse(defaultValue) as StatModifier[]; } catch { return []; }
  });
  return (
    <div>
